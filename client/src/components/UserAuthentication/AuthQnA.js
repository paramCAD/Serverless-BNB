...
export default function AuthQnA({ setIsQnAVerified }) {

    const [q1, SetQ1] = React.useState("Q1. Please provide your favorite sports person name ?")
    const [q2, SetQ2] = React.useState("Q2. Please provide your favorite food name ?")
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await axios.post(url, { email: email, ans1: a1, ans2: a2 });
            console.log(response.data);
            if (response.data === 'success') {
                setIsQnAVerified(true);
